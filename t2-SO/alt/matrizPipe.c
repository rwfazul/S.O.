...
double busca_fork(int *vet, int tam_vet, int qtd_pt) {
	...
	int fd[qtd_pt][2];		         // pipe file descriptor :  fd[x][0] leitura (saida), fd[x][1] escrita (entrada)
	for (int i = 0; i < qtd_pt; i++) {
		pid_t pid;
		if (pipe(fd[i]) < 0) {		// criacao pipe
			fprintf(stderr, "Pipe %d falhou\n", i);
			exit(-1);
		}
		if ( (pid = fork()) < 0) {
		...
		if (pid > 0)  {												//pai 
			...
			close(fd[i][1]);       // pai vai ler, logo fecha escrita do pipe
			read(fd[i][0], &ret_proc[i], sizeof(retorno));	// le  oq foi escrito no pipe e armazena no vetor de retornos
		}
		else {											 	        // filho
			close(fd[i][0]);     // filho vai escrever, logo fecha a leitura do pipe
			info[i].id_pt = getpid();
			info[i].vet = vet;
			info[i].indice_inicio =  fatia * i;
			info[i].indice_fim = ((i + 1) < qtd_pt) ? (info[i].indice_inicio +  fatia) : tam_vet;
			// se sobrar algum resto fica para o ultimo processo (par/impar)
			write(fd[i][1], (retorno*) procura_intervalo((void*) &info[i]), sizeof(retorno)); // escreve retorno da funcao procura_intervalo
			exit(1);            // fim do filho (evita voltar no loop)
		}	
	}

	/* Processo principal ("processo pai") imprime resultados de todos processos criados a partir dele*/
	for (int i = 0; i < qtd_pt; i++) 
		printf("Processo %d -> %d numeros primos!\n", ret_proc[i].id_pt + 1, ret_proc[i].qtd_primos);
